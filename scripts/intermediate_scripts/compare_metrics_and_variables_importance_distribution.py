...
def plot_feature_distribution(yvar='Atrophy_36m', N_SPLITS_INNER=6, N_SPLITS_OUTER=6):
    ...
    def plotPredDistributions(ytest,ypred,title,conf_mat):
        ...
        plt.savefig(f'figures/{yvar}_summary_figures_{variables_p}_{title}_pred_distribution_{yvar}_{variables_p}.pdf')
    ...
    all_combs = pd.read_csv(f'inputs/{yvar}_best_conf_{N_SPLITS_INNER}_{N_SPLITS_OUTER}.tsv',sep='\t')
    ...
    for title, best_conf_params_cond in zip(['MaxVal BalAcc','MaxVal AUROC'], 
                                            [bestConfValBalAcc, bestConfValAUROC]):
        ...
        if not os.path.exists(f'figures/{yvar}_summary_figures_{variables_p}'):
            os.mkdir(f'figures/{yvar}_summary_figures_{variables_p}')
        ...
        if not no_feature_imp:
            ...
            _ = plt.savefig(f'figures/{yvar}_summary_figures_{variables_p}_{title}_feature_imp_{yvar}_{variables_p}.pdf')

        
#Generate for a variable, the 2 best models according to ValBalAcc, ValAUROC
